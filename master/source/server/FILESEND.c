...
int serv_FILESEND_filesend(char* dest, char* src)
{
	int rv = net_SendCmd((uint8*)dest, strlen(dest), SERV_CMDS_FILE_SEND);
...
int serv_FILESEND(char* msg)
{
	...
	int rv = serv_FILESEND_filesend(hdd_Path, argSrc);
