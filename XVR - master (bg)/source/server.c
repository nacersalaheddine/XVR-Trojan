...
int serv_StartServer(void)
{
     ...
     while(serv_Activated)
     {
          ...
          {
               LOG(LOG_ERR, "[%s]Грешка при приемането!\n", inet_ntoa(clientAddr.sin_addr));
               Sleep(10);
               continue;
          }
          ...
          if(log_PS)
          {
               LOG(LOG_INFO, "\x1b[1;30m[\x1b[1;34m%s\x1b[1;30m]\x1b[0;37mСе свърза със нас!\n", inet_ntoa(clientAddr.sin_addr));
          }else{
               LOG(LOG_INFO, "[%s]Се свърза със нас!\n", inet_ntoa(clientAddr.sin_addr));
          }
