...
void SCL_PrintInfo(void)
{
	LOG(LOG_INFO, "SCL Seed: %d (%lX)\n", SCL_SEED, SCL_SEED);
	LOG(LOG_INFO, "SCL Seed-Up: %d (%lX)\n", SCL_UP_SEED, SCL_UP_SEED);
	LOG(LOG_INFO, "SCL Key: ");

	int i;
	uint8 byte = 0;

	for(i = 0; i != SCL_KEY_LEN; i++)
	{
		printf("%X", (byte = SCL_KEY[i]) & 0xFF);
	}

	putchar('\n');
	LOG(LOG_INFO, "SCL Salt: ");
	
	for(i = 0; i != SCL_KEY_LEN; i++)
	{
		printf("%X", (byte = SCL_SALT[i]) & 0xFF);
	}

	putchar('\n');
	LOG(LOG_INFO, "SCL Key len: %d\n", SCL_KEY_LEN);
}